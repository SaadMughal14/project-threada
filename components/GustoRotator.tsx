...
const PizzaSection: React.FC<{ 
  ...
  useGSAP(() => {
    gsap.to(pizzaRef.current, {
      ...
      scale: 1.02,
      scrollTrigger: { trigger: container.current, start: "top bottom", end: "bottom top", scrub: 1.2 }
    });
  }, { scope: container });
  ...
  return (
    <section 
      ...
      style={{ backgroundColor: pizza.color || '#1C1C1C' }}
    >
      <div className={`flex flex-col w-full max-w-5xl items-center gap-6 md:gap-16 z-10 ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
        <div className="flex-1 flex justify-center w-full">
          <div ref={pizzaRef} className="w-[50vw] h-[50vw] md:w-[25vw] md:h-[25vw] max-w-[260px] max-h-[260px] rounded-xl md:rounded-[2.5rem] shadow-xl border-[6px] md:border-[10px] border-white/5 overflow-hidden">
            <img src={pizza.image} alt={pizza.name} className="w-full h-full object-cover scale-[1.1]" />
          </div>
        </div>

        <div className={`flex-1 text-[#FDFCFB] space-y-3 md:space-y-6 text-center ${isEven ? 'md:text-left' : 'md:text-right'}`}>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {pizza.sizeOptions.map((option) => (
                <button
                  ...
                  className={`px-3 md:px-5 py-2 rounded-xl text-[8px] md:text-[9px] font-black uppercase tracking-widest transition-all border-2 ${selectedSize.name === option.name ? 'bg-[#D97B8D] border-[#D97B8D] text-[#1C1C1C]' : 'bg-white/5 border-white/10 text-white opacity-60'}`}
                >
                  {option.name}
                </button>
              ))}
